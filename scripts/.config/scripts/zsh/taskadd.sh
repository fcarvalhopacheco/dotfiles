...
# Load the file
source /Users/fcp/.dotfiles/scripts/.config/scripts/zsh/conf/taskconfig.sh
# Styling Function
echo_line() {
    echo "----------------------------------------------------"
}
...
confirm_and_add_task() {
    ...
    if [[ $input_confirm =~ ^[Yy]$ ]]; then
        ...
        if [[ "$project" != "PERSONAL" ]]; then
            task add "$task_desc" project:"$project"."$cruise_number"."$what"."$task_type" due:"$due_date" "$tags"
        else
